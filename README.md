...
[![Version](https://img.shields.io/badge/version-v1.6.0-blue null)](metadata.yaml)
...
- **证据保存** - 自动下载并保存违规图片证据
- **双群管理机制** - 管理群绑定多个被管理群时命令支持 `[群号|all]` 精确指定目标群，单绑定自动直达；人工名单支持群级范围；配置热重载无需重启
- **WebUI 管理面板** - Dashboard 内可视化查看违规记录/违规图片/LLM 分析/违规档案/审核状态统计，支持筛选排序与增删改查
...
> `group_name`（v1.5.0 新增）为可选群别名，用于 WebUI 与 `/审核状态` 统计展示，留空则显示群号。同一管理群可绑定多个被管理群（各条目的 `manage_group_id` 相同），管理群命令用法见下文「管理员命令」。
...
> **双群管理规则（v1.5.0）**：管理群仅绑定 1 个被管理群时，命令直接作用于该群（无需群号）；绑定多个时需带 `[群号|all]`，群号必须绑定到当前管理群。适用于 `/查询违规`、`/删除违规`、`/审核状态` 及名单类命令。

| 命令                         | 说明                   | 示例                      |
| -------------------------- | -------------------- | ----------------------- |
| `/查询违规 [QQ号] [群号\|all]`    | 查询用户违规记录             | `/查询违规 123456789`       |
| `/删除违规 [QQ号] [群号\|all] 确认` | 删除指定用户的违规记录（需确认）     | `/删除违规 123456789 all 确认` |
| `/审核状态 [群号\|all]`          | 查看插件状态与各群统计          | `/审核状态`                 |
| `/清除缓存`                    | 清除所有自动黑白名单缓存         | `/清除缓存`                 |
| `/查询名单`                    | 查询图片在黑白名单中的状态（需引用图片） | `/查询名单`                 |

### 人工白名单管理

名单命令末位的 `[群号|all]` 为范围参数：`all` 或缺省 = 全局生效，具体群号 = 仅该群生效（需绑定到当前管理群）；不写范围时旧用法完全兼容。

| 命令                      | 说明                | 示例                       |
| ----------------------- | ----------------- | ------------------------ |
| `/添加白名单 [原因] [群号\|all]` | 添加图片到人工白名单（需引用图片） | `/添加白名单 "正常图片" 987654321` |
| `/移除白名单 [群号\|all]`      | 从人工白名单移除图片（需引用图片） | `/移除白名单 987654321`       |
...
| 命令                                     | 说明                | 示例                             |
| -------------------------------------- | ----------------- | ------------------------------ |
| `/添加黑名单 [REVIEW/BLOCK] [原因] [群号\|all]` | 添加图片到人工黑名单（需引用图片） | `/添加黑名单 BLOCK "色情内容" 987654321` |
| `/移除黑名单 [群号\|all]`                     | 从人工黑名单移除图片（需引用图片） | `/移除黑名单`                       |
...
白名单中的 QQ 账号跳过图片审核。`[群号|all]` 为范围参数：缺省/`all` = 全局生效（添加）或全部范围（移除/列表），具体群号 = 仅该群（需绑定到当前管理群）。可用配置项 `enable_account_whitelist` 关闭此功能。

| 命令                       | 说明      | 示例                    |
| ------------------------ | ------- | --------------------- |
| `/添加账号白名单 <QQ号> [群号\|all]` | 添加账号到白名单 | `/添加账号白名单 123456789`  |
| `/移除账号白名单 <QQ号> [群号\|all]` | 从白名单移除账号 | `/移除账号白名单 123456789 987654321` |
| `/账号白名单列表 [群号\|all]`     | 查看白名单列表 | `/账号白名单列表`            |
...
在 AstrBot Dashboard 的**插件详情页**打开「图片审核管理」页面，提供 7 个功能标签：

| 标签    | 功能                                                                    |
| ----- | --------------------------------------------------------------------- |
| 概览    | 今日/累计审核与违规统计、审核状态数量（通过/复审/违规）、近 7 天趋势图、Top10 违规用户、各群分布                |
| 违规记录  | 查看**违规图片**（服务端压缩输出）与 **LLM 分析**全文；按群/用户/关键字/风险等级/日期筛选；排序、分页、编辑备注、批量删除 |
| 审核日志  | 全部图片审核记录（含通过）浏览与删除，自动保留 30 天                                          |
| 白名单   | 人工白名单管理（全局/群级范围筛选、添加、改备注、删除）                                          |
| 黑名单   | 人工黑名单管理（含风险等级展示）                                                      |
| 账号白名单 | QQ 账号白名单管理（全局/群级范围，添加、改备注、删除）                                          |
| 违规档案  | 跨群用户档案（昵称/状态/备注/首末次出现/违规计数），审核时自动建档，支持手动管理                            |
